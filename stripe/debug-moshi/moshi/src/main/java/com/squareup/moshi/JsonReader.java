...
public class JsonReader implements Closeable {
  ...
  private ParserStack ps = new ParserStack();
  ...
  private int doPeek() throws IOException {
    int peekStack = ps.getTop();
    if (peekStack == JsonScope.EMPTY_ARRAY) {
      ps.setTop(JsonScope.NONEMPTY_ARRAY);
      ...
      ps.setTop(JsonScope.DANGLING_NAME);
      // Look for a comma before the next element.
      ...
      ps.setTop(JsonScope.NONEMPTY_OBJECT);
      ...
      ps.setTop(JsonScope.NONEMPTY_DOCUMENT);
  ...
  public void skipValue() throws IOException {
    ...
    do {
      ...
      if (p == PEEKED_BEGIN_ARRAY) {
        ps.push(JsonScope.EMPTY_ARRAY);
        count++;
      } else if (p == PEEKED_BEGIN_OBJECT) {
        ps.push(JsonScope.EMPTY_OBJECT);
        count++;
